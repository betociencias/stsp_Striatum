...
if 0:
    pars={'timeMin':0.0, 'timeMax': 1000.0, 'timeStep':1e-3,
      'tauX':100.0,  'tauP':100.0, 
      'asynX':1.0, 'asynP':0.3, 'asynXPm':'ko',
      'kP':0.0, 'kX':1.0, 
      'jumpP':0.2, 'tauTrain':1.0, 
      'stimHz':20.0, 'stimStart':10.0}
    pars= stimTrains(pars)
    orbit=simulateFD(pars)
    r=1; c=1
...
# --------------------------------------------------
def phase2DynamicsMovie(data, pars, msFramePoints=3.1*20, msJumpPoints=10):
    peakInds = pars['peakInds']
    firstPeakInd=peakInds[0]
    relAmp= orbit[2][firstPeakInd]
    xpNorm=orbit[2][peakInds]/relAmp
    figStr1=r'$\tau_x$=%.0f, $\tau_p$=%.0f, $k_x$=%.1f, $k_p$=%.1f, $h$=%.1f, $x_{\infty}$=%.1f, $p_{\infty}$=%.1f'%(pars['tauX'], pars['tauP'], pars['kX'], pars['kP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    figName='taux=%.0f_taup=%.0f_kx=%.1f_kp=%.1f_h=%.1f_xa=%.1f_pa=%.1f'%(pars['tauX'], pars['tauP'], pars['kX'], pars['kP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    newPath="jjj" #+figName
    movieName=figName
    ...
    l3, = ax1.plot([], [], 'wo',ms=3)
    ax0.plot([pars['asynX'],], [pars['asynP'],], pars['asynXPm'], ms=4)
    ...
    ax1.set_xlim(0,pars['timeMax']);  ax1.set_ylim(-0.1,1.1*xpNorm.max())
    for k in range(0,nFrames):
        b=(k+1)*nPtsJump
        a= sc.maximum(b-nPtsPerFrame,0)
        if (b-1>nSimPts):
            b=nSimPts
        l0.set_data(data[0][a:b], data[1][a:b])
        l0.set_data(data[0][a:b], data[0][a:b]*data[1][a:b])
        l1.set_data(pars['sampleTimes'][a:b], data[0][a:b]*data[1][a:b])
        l2.set_data(pars['sampleTimes'][peakInds], xpNorm)
        l3.set_data(pars['sampleTimes'][peakInds], xpNorm)
        fig1.canvas.draw()
        fig1.savefig(newPath+"/"+figName+"_%03d.png"%k)

    l0.set_data(data[0], data[1])
    l0.set_data(data[0], data[0]*data[1])
    l1.set_data(pars['sampleTimes'], data[0]*data[1])
    fig1.canvas.draw()
            ...
            for h in sc.arange(0.1,1.0,0.1):
                ...
                for m in range(len(tau)): 
                    ...
                    for n in range(len(tau)): 
                        ...
                        pars= stimTrains(pars)
                        orbit= simulateFD(pars)
                        figName=phase2DynamicsMovie(data=orbit[0:2], pars=pars, msFramePoints=1.5*20, msJumpPoints=10)
...
# -----------------------------------------
def phase1DynamicsMovie(data, pars, msFramePoints=3.1*20, msJumpPoints=10, movieName='phase1Dynamics'):
    figStr1=r'$\tau_x$=%.0f, $\tau_p$=%.0f, $h$=%.1f, $x_{\infty}$=%.1f, $p_{\infty}$=%.1f'%(pars['tauX'], pars['tauP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    figName='taux=%.0f_taup=%.0f_h=%.1f_xa=%.1f_pa=%.1f'%(pars['tauX'], pars['tauP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    newPath="jjj"+figName
    if len(movieName)<1:
    ...
    for k in range(0,nFrames):
        ...
        if (b-1>nSimPts):
            b=nSimPts
        l.set_data(data[0][a:b], data[1][a:b])
        fig1.canvas.draw()
        fig1.savefig(newPath+"/"+figName+"_%03d.png"%k)
...
def dynamicsMovie(data, pars, msFramePoints=3.1*20, msJumpPoints=10,cm=['b.','g.','k.']):
    figStr1=r'$\tau_x$=%.0f, $\tau_p$=%.0f, $h$=%.1f, $x_{\infty}$=%.1f, $p_{\infty}$=%.1f'%(pars['tauX'], pars['tauP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    figName='taux=%.0f_taup=%.0f_h=%.1f_xa=%.1f_pa=%.1f'%(pars['tauX'], pars['tauP'], pars['jumpP'], pars['asynX'], pars['asynP'])
    newPath="jjj"+figName
    if not os.path.exists(newPath): 
    ...
    for k in range(0,nFrames):
        ...
        if (b-1>nSimPts):
            b=nSimPts
        for n in range(nVars):
            lines[n].set_data(pars['sampleTimes'][a:b], data[n][a:b])
        fig1.canvas.draw()
        fig1.savefig("jjj/fig%03d.png"%k)

    for n in range(nVars):
        lines[n].set_data(pars['sampleTimes'], data[n])
...
if 0: 
    dynamicsMovie(pars=pars, data= orbit, msFramePoints=3.1*20, msJumpPoints=10)
