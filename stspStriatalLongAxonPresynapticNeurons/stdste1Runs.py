...
if 0:
    ...
    f0= ajusteParametros(simData=sim1, expeData=dataVals, saveFig=1)
...
def phase2DynamicsMovie(simData, pa, msFramePoints=3.1*20, msJumpPoints=10):
    ...
    newPath1="jjj" #+figName
    newPath2="jjjmovies" #+figName
    movieName=figName
    ...
    for k in range(0,nFrames):
        ...
        if (b-1>nSimPts):
            b=nSimPts
        l0.set_data(simData['xOrbit'][a:b], simData['pOrbit'][a:b])
        l0.set_data(simData['xOrbit'][a:b], simData['xOrbit'][a:b]*simData['pOrbit'][a:b])
        l1.set_data(pa['sampleTimes'][a:b], simData['xOrbit'][a:b]*simData['pOrbit'][a:b])
    ...
    l0.set_data(simData['xOrbit'], simData['pOrbit'])
    l0.set_data(simData['xOrbit'], simData['xOrbit']*simData['pOrbit'])
    l1.set_data(pa['sampleTimes'], simData['xOrbit']*simData['pOrbit'])
    fig1.canvas.draw()
    fig1.suptitle(figStr1, fontsize=10)
    thisFigName=newPath1+"/"+figName+"_%03d.png"%k
    fig1.savefig(thisFigName)
    print 'Making movie %s.mpg - this make take a while'%figName
    os.system("mencoder 'mf://%s/%s*.png' -mf type=png:fps=5 -ovc lavc -lavcopts vcodec=wmv2 -oac copy -o %s/%s.mpg"%(newPath1,figName,newPath2, figName))
    for k in range(0,nFrames-1):
        os.system("rm '%s/%s_%03d.png'"%(newPath1,figName,k)
    plt.close('all')
    return figName
            ...
            for h in sc.arange(0.1,1.0,0.1):
                ...
                for m in range(len(tau)): 
                    ...
                    for n in range(len(tau)): 
                        pars['tauP']=tau[n]
                        pars= stimTrains(pars)
                        simul= simulateFD(pars)
                        figName=phase2DynamicsMovie(simData=simul, pa=pars, msFramePoints=1.5*20, msJumpPoints=10)
    totTime =time.time()-startTime
    totTimeMins=totTime/60.0
    totTimeHrs=totTimeMins/60.0
    print "Took %f seconds (%f minutes, %f hours) to finish the job"%(totTime,totTimeMins,totTimeHrs)
...
def phase1DynamicsMovie(simData, pars, msFramePoints=3.1*20, msJumpPoints=10, movieName='phase1Dynamics'):
    ...
    for k in range(0,nFrames):
        ...
        if (b-1>nSimPts):
            b=nSimPts
        l.set_data(simData['xOrbit'][a:b], simData['pOrbit'][a:b])
        fig1.canvas.draw()
        fig1.savefig(newPath+"/"+figName+"_%03d.png"%k)

    l.set_data(simData['xOrbit'], simData['pOrbit'])
    fig1.canvas.draw()
    fig1.suptitle(figStr1, fontsize=10)
    fig1.savefig(newPath+"/"+figName+"_%03d.png"%k)
